...
void SHAify::add(unsigned char *data, unsigned int datalen)
{
	...
	for (i = 0; i < datalen; i++) {
		...
		if (!(++lenW & 63)) {
			...
			for (t = 0; t < 20; t++) {
				unsigned int TEMP = SHA_ROTL(A,5) + E + W[t] + 0x5a827999L + (((C^D)&B)^D);
				SHUFFLE();
			};
			for (; t < 40; t++) {
				unsigned int TEMP = SHA_ROTL(A,5) + E + W[t] + 0x6ed9eba1L + (B^C^D);
				SHUFFLE();
			};
			for (; t < 60; t++) {
				unsigned int TEMP = SHA_ROTL(A,5) + E + W[t] + 0x8f1bbcdcL + ((B&C)|(D&(B|C)));
				SHUFFLE();
			};
			for (; t < 80; t++) {
				unsigned int TEMP = SHA_ROTL(A,5) + E + W[t] + 0xca62c1d6L + (B^C^D);
				SHUFFLE();
			};
