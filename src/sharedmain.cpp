			...
			if (!memcmp(r->get_guid(),&g_client_id,sizeof(g_client_id))) {
					...
					if (!memcmp(r->get_prev_guid(),xs->get_guid(),16)) {
						...
						if (r->is_abort()==2) {
							...
							if (a >= 0 && a < g_uploads.GetSize()) {
								...
								if (p) {
									intptr_t lvidx;
									while ((lvidx=g_lvsend.FindItemByParam((intptr_t)p)) >= 0) {
				...
				if (x == n && (r->is_abort()==3)) { //file exists
					...
					if (idx >= UPLOAD_BASE_IDX) {
						idx-=UPLOAD_BASE_IDX;
						intptr_t lvidx=g_lvsend.FindItemByParam((intptr_t)g_uploads.Get(idx));
						if (lvidx >= 0) {
						...
						if (idx >= 0) {
							if (idx < UPLOAD_BASE_IDX) {
...
#endif
								{
									...
									intptr_t lvidx;
									if ((lvidx=g_lvsend.FindItemByParam((intptr_t)g_uploads.Get(idx))) >= 0) {
										g_lvsend.DeleteItem(lvidx);
									};
#endif
								};
...
#error todo
							...
							if (err) {
								...
								g_lvsend.InsertItem(0,a->GetName(),(intptr_t)a);
								char buf[32];
								int fs_l,fs_h;
								a->GetSize((unsigned int *)&fs_l,(unsigned int *)&fs_h);
								FormatSizeStr64(buf,fs_l,fs_h);
