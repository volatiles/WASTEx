...
bool allowIP(unsigned int addr)
{
...
int ACStringToStruct(const char *t, ACitem *i)
{
	...
	t++;
	char *p=strstr(const_cast<char*>(t),"/");
	if (!p || !p[1]) return 0;
	...
	if (!err) {
		...
		if (p && atoi(p) >= 10 && atoi(p) < 20) {
			...
			if (p && (key->bits=atoi(p)) <= MAX_RSA_MODULUS_BITS && key->bits >= MIN_RSA_MODULUS_BITS) {
				...
				if (!err) {
					char buf[8];
					bl.SetIV(CBlowfish::IV_BOTH,(unsigned int*)tl);
...
bool IPv4TestIpInMask(unsigned int addr,unsigned int subnet,unsigned int mask)
{
	subnet&=mask;
	addr&=mask;
	return addr==subnet;
...
unsigned int IPv4Addr(unsigned char i1,unsigned char i2,unsigned char i3,unsigned char i4)
{
	unsigned int t;
	t=htonl(
		(((unsigned int)i1)<<24)|
...
unsigned int IPv4NetMask(unsigned int networkbits)
{
	unsigned int i;
...
bool IPv4IsLoopback(unsigned int addr)
{
	return IPv4TestIpInMask(addr,IPv4Addr(127,  0,  0,  0),IPv4NetMask( 8));
}

bool IPv4IsPrivateNet(unsigned int addr)
{
	bool t;
...
#ifdef _WIN32
	bool GetInterfaceInfoOnAddr(unsigned int addr,unsigned int &localaddr,unsigned int &localmask)
	{
		SOCKET s;
		...
		for (ifa = ifaces; ifa != NULL; ifa = ifa->ifa_next)
		{
			if ((ifa->ifa_flags & IFF_UP)!=0 && ifa->ifa_addr != NULL)
				...
				if (ifa->ifa_addr->sa_family==AF_INET) {
					...
					unsigned int laddr=in->sin_addr.s_addr;
					in=(sockaddr_in*)(ifa->ifa_netmask);
					unsigned int lmask=in->sin_addr.s_addr;
					if (IPv4TestIpInMask(addr,laddr,lmask)) {
						ret=true;
						break;
					};
...
bool is_accessable_addr(unsigned int addr)
{
	if (g_use_accesslist) return true;
...
void update_forceip_dns_resolution()
{
	if (g_forceip_dynip_mode==1) {
		unsigned int ipold=g_forceip_dynip_addr;
		unsigned int ip=inet_addr(g_forceip_name);
		if (ip==INADDR_NONE) {
...
void GenerateRandomSeed(void* target,unsigned int len,R_RANDOM_STRUCT *random)
{
	...
	unsigned int tm[2]={time(NULL),GetTickCount()};
...
//ADDED Md5Chap
	...
	#define _BN1 1
	//----------------------------------------------------------------------------------
	static unsigned int CheckRsa_Single(int enable,int n,unsigned char *inp,bool blind)
	{
	...
	void CheckRsaBlinding()
	{
		...
		if (g_key.bits>0) {
			...
			//------------------------------
			int n=N;
			unsigned int tick1u=CheckRsa_Single(_UQ0,n,cq0,false);
			unsigned int tick2u=CheckRsa_Single(_UQ1,n,cq1,false);
			unsigned int tick3u=CheckRsa_Single(_UN1,n,cn1,false);
			unsigned int tick1b=CheckRsa_Single(_BP0,n,cq0,true);
			unsigned int tick2b=CheckRsa_Single(_BP1,n,cq1,true);
			unsigned int tick3b=CheckRsa_Single(_BN1,n,cn1,true);
			//------------------------------
