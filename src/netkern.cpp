		...
		if (c) {
			...
			if (!allowIP(addr)) {
				...
				c->send_bytes(g_con_str,SYNC_SIZE);
					...
					g_lvnetcons.InsertItem(0,"Authenticating",(intptr_t)c);
...
void NetKern_ConnectToHostIfOK(unsigned int ip, unsigned short port)
{
	struct in_addr in;
	in.s_addr=ip;
	char *t=inet_ntoa(in);
	...
	if (is_accessable_addr(ip) && allowIP(ip)) {
		...
		g_new_net.Add(newcon);
			...
			if (netqpos<0) {
				...
				g_lvnetcons.InsertItem(0,"Connecting",(intptr_t)newcon);
				g_lvnetcons.SetItemText(0,1,buf);
				g_lvnetcons.SetItemText(0,2,"90");
			}
			else {
				g_lvnetcons.SetItemParam(netqpos,(intptr_t)newcon);
...
static void SendCaps(C_MessageQueue *mq)
{
	if (mq) { //send caps
		...
		unsigned int rip=mq->get_con()->get_remote();
		if (rip) {
...
static void HandleNewOutCons()
{
	...
	for (x = 0; x < g_new_net.GetSize(); x ++) {
		...
		if (s == C_Connection::STATE_DIENOW) {
			#if defined(_WIN32)&&(!defined(_DEFINE_SRV)) || defined(_DEFINE_WXUI)
				int a=g_lvnetcons.FindItemByParam((intptr_t)cc);
				if (a >= 0) {
			...
			PostMessageTitleUpdate(g_netstatus_wnd);
				...
				int irat=0;
				int a=g_lvnetcons.FindItemByParam((intptr_t)g_new_net.Get(x));
				if (a >= 0) {
					char rat[32];
