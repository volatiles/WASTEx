...
#else
	static forceinline unsigned int SWAP(unsigned int x)
	{
...
#define ISALIGN(x,y) ((((intptr_t)x)&(y-1))==0)
...
static void forceinline Blowfish_Encrypt(_BLOWFISH_CTX *ctx, unsigned int *xl, unsigned int *xr)
{
	unsigned int Xl=*xl, Xr=*xr;
...
static void forceinline Blowfish_Decrypt(_BLOWFISH_CTX *ctx, unsigned int *xl, unsigned int *xr)
{
	unsigned int Xl=*xl, Xr=*xr;
	unsigned int *p=ctx->P+16+1;
	unsigned int *s=ctx->S;
	Xl = Xl ^ *p--;
	unsigned int i;
	for (i=0;i<(16/2);i++) {
...
static void Blowfish_Init(_BLOWFISH_CTX *ctx, const unsigned char *key, unsigned int keyLen)
{
	...
	{
		p=ctx->P;
		const unsigned int *p0=ORIG_P;
		unsigned int j=0;
		if (ISLONGALIGN(key)&&ISLONGALIGN(keyLen)) {
			unsigned int *pulkey=(unsigned int*)key;
			unsigned int *pulmaxkey=(unsigned int*)(key+keyLen);
			for (i=0;i<(16+2);i++) {
				*p++ = *p0++ ^ SWAP(*pulkey++);
				if (pulkey>=pulmaxkey) pulkey=(unsigned int*)key;
			};
		}
		else {
			unsigned int data=0;
...
public:
	...
	static forceinline void Encrypt(_BLOWFISH_CTX *ctx,unsigned int *IV_L,unsigned int *IV_R,unsigned int *left,unsigned int *right)
	{
		(void) IV_L; (void) IV_R;
		Blowfish_Encrypt(ctx,left,right);
	};
	static forceinline void Decrypt(_BLOWFISH_CTX *ctx,unsigned int *IV_L,unsigned int *IV_R,unsigned int *left,unsigned int *right)
	{
		(void) IV_L; (void) IV_R;
...
public:
	...
	static forceinline void Decrypt(_BLOWFISH_CTX *ctx,unsigned int *IV_L,unsigned int *IV_R,unsigned int *left,unsigned int *right)
	{
		unsigned int tl=*left;
		unsigned int tr=*right;
		Blowfish_Decrypt(ctx,left,right);
		*left ^=*IV_L;
		*right^=*IV_R;
		*IV_L=tl;
...
public:
	...
	static forceinline void DoOp(_BLOWFISH_CTX *ctx,unsigned int *IV_L,unsigned int *IV_R,unsigned int *left,unsigned int *right)
	{
...
{
public:
	static forceinline void DoBlock(_BLOWFISH_CTX *ctx, void *data, unsigned int len,unsigned int IV[2],bool updateIV)
	{
		...
		unsigned int IV_L=IV[0];
		unsigned int IV_R=IV[1];
		unsigned char *cleft  =(unsigned char*)data;  unsigned int *&lleft =(unsigned int *&)cleft;
		unsigned char *cright =(unsigned char*)data+4;unsigned int *&lright=(unsigned int *&)cright;
...
{
	public:
		static void Encrypt(_BLOWFISH_CTX *ctx, void *data, unsigned int len,unsigned int IV[2],bool updateIV);
		static void Decrypt(_BLOWFISH_CTX *ctx, void *data, unsigned int len,unsigned int IV[2],bool updateIV);
		typedef opmodeenc<opmode> encmode;
		typedef opmodedec<opmode> decmode;
};

template<class opmode> forceinline void BlockOp<opmode>::Encrypt(_BLOWFISH_CTX *ctx, void *data, unsigned int len,unsigned int IV[2],bool updateIV)
{
	BlockOpUni<encmode>::DoBlock(ctx,data,len,IV,updateIV);
}

template<class opmode> forceinline void BlockOp<opmode>::Decrypt(_BLOWFISH_CTX *ctx, void *data, unsigned int len,unsigned int IV[2],bool updateIV)
{
...
void CBlowfish::SetIV(_eIV which,unsigned int IV[2])
{
	ASSERT(m_bInited);
...
void CBlowfish::SetIV(_eIV which,unsigned int left,unsigned int right)
{
	ASSERT(m_bInited);
	if (which==IV_BOTH||which==IV_ENC) {
		m_IV_Enc[0]=left;
...
void CBlowfish::GetIV(_eIV which,unsigned int IV[2])
{
	ASSERT(m_bInited);
...
void CBlowfish::GetIV(_eIV which,unsigned int &left,unsigned int &right)
{
	ASSERT(m_bInited);
	ASSERT(which==IV_ENC||which==IV_DEC);
	if (which==IV_ENC) {
